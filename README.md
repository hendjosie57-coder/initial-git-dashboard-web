...
module graph of the highest-churn files (sized by cyclomatic complexity and
colored on a green → amber → red complexity ramp), per-file history analytics,
a side-by-side legacy/modernized code comparison built on Monaco, and a
blame-grounded chat assistant.
...
3. **Contextual blame chat** — with a file selected, ask the assistant
   ...
   file path, your message, and (for the quick-action pills) a `quickAction`
   tag to `/api/v1/chat/contextual-blame`; the backend runs `git blame`,
   isolates the lines relevant to your question, and the markdown reply
   streams into the chat window with the blame records used. The three
   quick actions each get a purpose-built reply from the deterministic
   analyst: **Explain intent** → an *Author intent* summary (primary owner
   of the relevant region plus what the commit trail says), **Find
   weaknesses** → a *Risk review* (the commit concentrating the most change
   in the relevant lines, plus commits and authors to scrutinize), and
   **History** → a *History lookup* (relevant lines, authors, and commits
   involved). With `ANTHROPIC_API_KEY` set, all replies are generated by a
   real LLM grounded in the same blame records instead.
